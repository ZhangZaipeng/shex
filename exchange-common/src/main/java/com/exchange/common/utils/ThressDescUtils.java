...
public class ThressDescUtils {
   ...
   * 3DES加密
   *
   * @param plainTexchanget 普通文本
   */
  public static String encode(String plainTexchanget, String iv, String secretKey) {
    ...
    try {
      ...
      encryptData = cipher.doFinal(plainTexchanget.getBytes(encoding));
   ...
   * 3DES解密
   *
   * @param encryptTexchanget 加密文本
   */
  public static String decode(String encryptTexchanget, String iv, String secretKey) {
    try {
      DESedeKeySpec spec = new DESedeKeySpec(secretKey.getBytes());
      SecretKeyFactory keyfactory = SecretKeyFactory.getInstance("desede");
      Key deskey = keyfactory.generateSecret(spec);
      ...
      byte[] decryptData = cipher.doFinal(Base64.decode(encryptTexchanget));
      return new String(decryptData, encoding);
    } catch (exchangeception e) {
      logger.info("des decode error encryptTexchanget:{};iv:{};secretKey:{}", e, encryptTexchanget, iv,
          secretKey);
      return "";
    }
  }
