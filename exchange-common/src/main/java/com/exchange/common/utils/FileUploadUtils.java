...
//import org.csource.common.Myexchangeception;
...
public class FileUploadUtils {
    ...
    *//**
     * 获取 StorageClient1
     *//*
    public static StorageClient1 getStorageClient() throws IOexchangeception {
    ...
    public static UploadResult uploadFile(MultipartFile multipartFile,
                                          List<String> allowexchangetends) throws IOexchangeception {
        if (multipartFile == null || multipartFile.getSize() == 0) {
            throw new IOexchangeception("上传文件不存在");
        }

        // 原始文件名
        String fileName = multipartFile.getOriginalFilename();
        if (StringUtils.isNullOrEmpty(fileName)) {
            throw new IOexchangeception("非法文件");
        }

        // 文件大小
        long fileSize = multipartFile.getSize();
        if (multipartFile.getSize() > Integer.parseInt(ConfigUtils.getString("upload.file.maxsize")) * 1024) {
            throw new IOexchangeception("上传文件不能超过  " + ConfigUtils.getString("upload.file.maxsize") + " K");
        }

        String fileexchanget = "";
        int suffixPos = fileName.lastIndexchangeOf(".");
        if (suffixPos > 0) {
            fileexchanget = fileName.substring(suffixPos + 1);
        }
        if (!allowexchangetends.contains(fileexchanget.toLowerCase())) {
            throw new IOexchangeception("非法文件格式" + fileexchanget.toLowerCase());
        }

        // dfs 文件路径
        String dfsFilePath = getDfsPath(multipartFile.getInputStream(), fileexchanget);
    ...
    private static String getDfsPath(InputStream inputStream, String fileexchanget) throws IOexchangeception{
        ...
        try {
            ...
            dfsFilePath = client1.upload_file1(file_buff, fileexchanget, null);
    ...
    *//**
     ...
     *//*
    public static UploadResult uploadImage(MultipartFile multipartFile) throws IOexchangeception {
        ...
        // 格式判断
        if (!ConfigUtils.getString("upload.img.file_exchangetension").contains(fileexchanget.toLowerCase())) {
            throw new IOexchangeception("非法文件格式" + fileexchanget.toLowerCase());
        }
    ...
    *//**
     ...
     * @throws IOexchangeception
     *//*
    public static List<UploadResult> uploadMultImages(MultipartFile[] multipartFiles) throws IOexchangeception {
    ...
    *//**
     ...
     *//*
    public static Integer delete_file(String storagePath) throws IOexchangeception{
        int result=-1;
        StorageClient1 client1 = getStorageClient();
        try {
            result = client1.delete_file1(storagePath);
        } catch (Myexchangeception e) {
            e.printStackTrace();
        }
