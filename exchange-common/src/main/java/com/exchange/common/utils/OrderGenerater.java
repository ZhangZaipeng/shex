...
public class OrderGenerater {
  ...
  public static String getCode(int length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      int indexchange = random.nexchangetInt(CHARNUM.length());
            /*if (i % 4 == 0)
                sb.append(" " + CHARNUM.charAt(indexchange));
            else*/
      sb.append(CHARNUM.charAt(indexchange));
    }
  ...
  public static String merchandiseNumber() {
    ...
    for (int i = 0; i < nums.length; i++) {
      // 将随机选取的数字存入目标数组
      int indexchange = random.nexchangetInt(canBeUsed);
      nums[i] = defaultNums[indexchange];
      // 将已用过的数字扔到备选数组最后，并减小可选区域
      swap(indexchange, canBeUsed - 1, defaultNums);
      canBeUsed--;
    }
    for (int i = 0; i < nums.length; i++) {
  ...
  public static String getCode(int length, String code, int maxCount) {
    ...
    if (StringUtils.isEmpty(code)) {
                /*for (int i = 0; i < length; i++) {
                    Random random = new Random();
                    int indexchange = random.nexchangetInt(CHARARTERS[0].length());//前20位随机取，后16按递增方式取，以避免重复
                    if(i<4){//前4位随机生成
                        sb.append(CHARARTERS[0].charAt(indexchange));
                    }else{//后面位，递增生成
        ...
        for (int k = 0; k < CHARARTERS[1].length() && count < maxCount; k++) {
          ...
          for (int m = 0; m < CHARARTERS[1].length() && count < maxCount; m++) {
            ...
            for (int n = 0; n < CHARARTERS[1].length() && count < maxCount; n++) {
              ...
              Random random = new Random();
              for (int e = 0; e < 4; e++) {
                int indexchange1_ = random.nexchangetInt(CHARARTERS[0].length());//前20位随机取，后16按递增方式取，以避免重复
                stringBuffer1
                    .insert(random.nexchangetInt(stringBuffer1.length()), CHARARTERS[0].charAt(indexchange1_));
              }
                                /*str = stringBuffer1.toString()+a+b+c+d;
                                int len = str.length();
                                for(int f = 0;f<len;f++){
                                    int indexchange = random.nexchangetInt(str.length());
                                    StringBuffer2.append(str.charAt(indexchange));
                                    str = str.replaceFirst(str.charAt(indexchange)+"", "");
                                }*/
      ...
      int indexchange1 = -1, indexchange2 = -1, indexchange3 = -1, indexchange4 = -1;
      for (int a = 0; a < code.length(); a++) {
        char letter = code.charAt(a);
        if (CHARARTERS[1].indexchangeOf(letter) >= 0) {
          if (indexchange1 == -1) {
            indexchange1 = CHARARTERS[1].indexchangeOf(letter);
          } else {
            if (indexchange2 == -1) {
              indexchange2 = CHARARTERS[1].indexchangeOf(letter);
            } else {
              if (indexchange3 == -1) {
                indexchange3 = CHARARTERS[1].indexchangeOf(letter);
              } else {
                if (indexchange4 == -1) {
                  indexchange4 = CHARARTERS[1].indexchangeOf(letter) + 1;
                }
              }
            }
          }
      ...
      log.info(
          "indexchange1=" + indexchange1 + ",indexchange2=" + indexchange2 + ",indexchange3=" + indexchange3 + ",indexchange4=" + indexchange4);
      int count = 0;
      for (int j = indexchange1; j < CHARARTERS[1].length() && count < maxCount; j++) {
        char a = CHARARTERS[1].charAt(j);
        for (int k = indexchange2; k < CHARARTERS[1].length() && count < maxCount; k++) {
          char b = CHARARTERS[1].charAt(k);
          for (int m = indexchange3; m < CHARARTERS[1].length() && count < maxCount; m++) {
            char c = CHARARTERS[1].charAt(m);
            for (int n = indexchange4; n < CHARARTERS[1].length() && count < maxCount; n++) {
//                                sb.append(a).append(b).append(c).append(CHARARTERS[1].charAt(n)).append(",");
              ...
              Random random = new Random();
              for (int e = 0; e < 4; e++) {
                int indexchange_1 = random.nexchangetInt(CHARARTERS[0].length());//前20位随机取，后16按递增方式取，以避免重复
                sb1
                    .insert(random.nexchangetInt(sb1.length()), CHARARTERS[0].charAt(indexchange_1));
                                    /*sb1.append(CHARARTERS[0].charAt(indexchange1));
                                    int indexchange_2 = random.nexchangetInt(sb1.length());
                                    StringBuffer2.append(CHARARTERS[0].charAt(indexchange_1)).append(sb1.charAt(indexchange_2));*/
              }
